...
URL_TEMPLATE = "https://raw.githubusercontent.com/CrossDarkrix/Pythonista3_pip_Configration_Tool/main/StaSh/stash-v1.0.zip"
...
UNWANTED_FILES = [
    ...
    "bin/cls.py",
    "stash.py",
    "lib/librunner.pysystem/shui.py",
...
def main(defs={}):
    ...
    os.makedirs(
        os.path.join(BASE_DIR, "Documents", "site-packages", "_bin"), exist_ok=True
    )
    shutil.copy(
        os.path.join(BASE_DIR, "Documents", "launch_stash.py"),
        os.path.join(BASE_DIR, "Documents", "site-packages", "PyTermianl3.py"),
    )
    with open(
        os.path.join(BASE_DIR, "Documents", "site-packages", "_bin", "sh"), "w"
    ) as f1:
        f1.write("from PyTermianl3 import main\nmain()")
    with open(
        os.path.join(BASE_DIR, "Documents", "site-packages", "_bin", "terminal"), "w"
    ) as f2:
